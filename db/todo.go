...
//
// ANSWER:  This is like keeping things private in C -
//
//	So by keeping items from being exported helps foster encapsulation, reducing dependency, improved mutability and security.
type ToDo struct {
	toDoMap    DbMap
	dbFileName string
}
...
func (t *ToDo) AddItem(item ToDoItem) error {
	...
	if err != nil {
		return errors.New("AddItem() LoadDB failed")
	}
	...
	dberr := t.saveDB()
	if dberr != nil {
		return errors.New("AddItem() saveDB failed")
	}
...
func (t *ToDo) DeleteItem(id int) error {
	...
	if err != nil {
		return errors.New("DeleteItem() LoadDB failed")
	}
	...
	dberr := t.saveDB()
	if dberr != nil {
		return errors.New("DeleteItem() saveDB failed")
	}
...
func (t *ToDo) UpdateItem(item ToDoItem) error {
	...
	if err != nil {
		return errors.New("UpdateItem() LoadDB failed")
	}

	if _, ok := t.toDoMap[item.Id]; ok {
		t.toDoMap[item.Id] = item
	} else {
		return errors.New("UpdateItem() Item does not exist ")
	}
...
func (t *ToDo) GetItem(id int) (ToDoItem, error) {
	...
	if err != nil {
		return ToDoItem{}, errors.New("GetItem() LoadDB failed")
	}
...
func (t *ToDo) GetAllItems() ([]ToDoItem, error) {
	...
	err := t.loadDB()
	if err != nil {
		return nil, errors.New("GetItem() LoadDB failed")
	}

	v := make([]ToDoItem, 0, len(t.toDoMap))
